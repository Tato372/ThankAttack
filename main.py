...
for x in range(constantes.TIPOS_TILES):
    imagen_tile = pygame.image.load(f"assets//images//tiles//mapa_thank_attack_{x+1}_32x32.png")
...
#Lista que representa y almacena los datos del mundosa
data_mundo = []
obstaculos_mundo = []

for fila in range(constantes.FILAS):
    filas = [282] * constantes.COLUMNAS
    data_mundo.append(filas)
    
##Cargar los archivos con el nivel
with open("niveles//mapa_nivel_1.csv", newline='') as csvfile:
...
#Crear el mundo
mundo = Mundo()
mundo.process_data(data_mundo, lista_tiles, imagenes_items, animaciones_enemigos)
...
#Crear el arma del tanque
cañon_tanque = Weapon(imagen_cañon, imagen_balas)
...
while run:
    if mostrar_inicio:
        ...
        if tanque_jugador.vivo:
                ...
                for tanque in tanques_enemigos:
                    if not tanque.update():  
                        ...
                        tanque.tanques_enemigos(posicion_pantalla, mundo.obstaculos_tiles, tanque_jugador, grupo_balas_enemigas, tanques, data_mundo)
                        tanque.update()
                
                #Actualiza estado del arma
                bala = cañon_tanque.uptade(tanque_jugador, 1, False)
                if bala:
                ...
                for tanque in tanques_enemigos:
                    tanque.update_cañon_enemigo(tanque, cañon_tanque, mundo.obstaculos_tiles, tanque_jugador, grupo_balas_enemigas)
        ...
        #Dibujar al tanque del jugador
        tanque_jugador.dibujar(pantalla)
        ...
        #Dibujar el arma del tanque
        cañon_tanque.dibujar(pantalla)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if boton_reinicio.collidepoint(event.pos) and tanque_jugador.vivo == False:
                    tanques = []
                    data_mundo = resetear_mundo()
                    ##Cargar los archivos con el nivel
                    with open("niveles//mapa_nivel_1.csv", newline='') as csvfile:
                        reader = csv.reader(csvfile, delimiter=',')
                        for x, fila in enumerate(reader):
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if boton_reinicio_vic.collidepoint(event.pos) and tanques_enemigos == []:
                    tanques = []
                    data_mundo = resetear_mundo()
                    ##Cargar los archivos con el nivel
                    with open("niveles//mapa_nivel_1.csv", newline='') as csvfile:
